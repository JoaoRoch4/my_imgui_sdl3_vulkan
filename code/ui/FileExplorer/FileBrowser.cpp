...
ImGui::FileBrowser::FileBrowser(ImGuiFileBrowserFlags flags,
    ...
    , keepOpen_(false) {
	assert(!((flags_ & ImGuiFileBrowserFlags_SelectDirectory) && (flags_ & ImGuiFileBrowserFlags_EnterNewFilename)) && "'EnterNewFilename' doesn't work when 'SelectDirectory' is enabled");
...
ImGui::FileBrowser& ImGui::FileBrowser::operator=(const FileBrowser& copyFrom) {
	...
	sortField_ = copyFrom.sortField_;
	sortAscending_ = copyFrom.sortAscending_;
...
void ImGui::FileBrowser::Display() {
	...
	if (editDir_) {
		...
		PushItemWidth(-1);
		const bool enter = InputText("##directory", currDirBuffer_.data(), currDirBuffer_.size(),
	...
	{
		static const char* sortLabels[] = {"Name", "Type", "Size", "Modified"};
		TextDisabled("Sort:");
		SameLine();
		SetNextItemWidth(110.0f);
		int sortIdx = static_cast<int>(sortField_);
		if (Combo("##fb_sort", &sortIdx, sortLabels, 4)) {
			sortField_ = static_cast<SortField>(sortIdx);
			UpdateFileRecords();
		}
		SameLine();
		static const char* dirLabels[] = {"Ascending", "Descending"};
...

	{
		...
		const bool shouldHideRegularFiles = (flags_ & ImGuiFileBrowserFlags_HideRegularFiles) && (flags_ & ImGuiFileBrowserFlags_SelectDirectory);
			...
			if (BeginTable("##fb_grid", cols)) {
				...
				for (unsigned int rscIdx = 0; rscIdx < fileRecords_.size(); ++rscIdx) {
					...
					if (!rsc.isDir && !isMediaMatched(rsc.extension))
						continue;
					if (!rsc.isDir && !tagFilter_.empty() && std::find(rsc.tags.begin(), rsc.tags.end(), tagFilter_) == rsc.tags.end())
						continue;
					...
					if (hovered && IsMouseClicked(ImGuiMouseButton_Left)) {
						...
						if (canSelect) {
							const bool multiSel = (flags_ & ImGuiFileBrowserFlags_MultipleSelection) != 0 && IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows);
		...
		if (!useGridView) {
			for (unsigned int rscIndex = 0; rscIndex < fileRecords_.size();
				...
				if (Selectable(rsc.showName.c_str(), selected, selectableFlag,
					...
					const bool canSelect = rsc.name != ".." && rsc.isDir == wantDir;
					const bool rangeSelect = canSelect && GetIO().KeyShift && rangeSelectionStart_ < fileRecords_.size() && (flags_ & ImGuiFileBrowserFlags_MultipleSelection) && IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows);
					const bool multiSelect = !rangeSelect && GetIO().KeyCtrl && (flags_ & ImGuiFileBrowserFlags_MultipleSelection) && IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows);
	...
	if (!focusOnInputText && !editDir_) {
		const bool selectAll = (flags_ & ImGuiFileBrowserFlags_MultipleSelection) && IsKeyPressed(ImGuiKey_A) && (IsKeyDown(ImGuiKey_LeftCtrl) || IsKeyDown(ImGuiKey_RightCtrl));
		if (selectAll) {
	...
	const bool isEnterPressed = (flags_ & ImGuiFileBrowserFlags_ConfirmOnEnter) && IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) && IsKeyPressed(ImGuiKey_Enter);
	...
	const bool doClose = Button("cancel") || shouldClose_ || ((flags_ & ImGuiFileBrowserFlags_CloseOnEsc) && IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) && IsKeyPressed(ImGuiKey_Escape));
	if (doClose) {
		closeContainer();
	}
...
int ImGui::FileBrowser::GetSortModeIndex() const noexcept {
	return static_cast<int>(sortField_);
}
...
bool ImGui::FileBrowser::GetSortAscending() const noexcept {
...
void ImGui::FileBrowser::SetInputName(std::string_view input) {
	assert((flags_ & ImGuiFileBrowserFlags_EnterNewFilename) && "SetInputName can only be called when "
...
void ImGui::FileBrowser::UpdateFileRecords() {
					...
					if (getxattr(p.path().c_str(), "user.xdg.tags", raw.data(), raw.size()) >= 0) {
						...
						while (s < raw.size()) {
							...
							while (!tag.empty() && (tag.back() == ' ' || tag.back() == '\t'))
								tag.pop_back();
							if (!tag.empty())
								rcd.tags.push_back(std::move(tag));
							s = e + 1;
						}
					}
				}
	...
	if (fileRecords_.size() > 2) {
		std::sort(fileRecords_.begin() + 1, fileRecords_.end(),
			[this](const FileRecord& a, const FileRecord& b) -> bool {
				...
				const int cmp = CompareByField(a, b);
				if (cmp != 0)
	...
	// Rebuild the set of tags present in this directory (for the filter combo).
	{
		...
		if (!tagFilter_.empty() && std::find(availableTags_.begin(), availableTags_.end(), tagFilter_) == availableTags_.end())
			tagFilter_.clear();
	}
...
int ImGui::FileBrowser::CompareByField(const FileRecord& a,
	...
	const auto threeWay = [](const auto& x, const auto& y) -> int {
		if (x < y)
			return -1;
		if (y < x)
			return 1;
	...
	return threeWay(ToLower(u8StrToStr(a.name.u8string())),
...
void ImGui::FileBrowser::SetCurrentDirectoryUncatched(
	...
	if ((flags_ & ImGuiFileBrowserFlags_EnterNewFilename) && selectedFilenames_.size() == 1 && !customizedInputName_.empty() && !inputNameBuffer_.empty() && std::strcmp(inputNameBuffer_.data(), customizedInputName_.data()) == 0) {
