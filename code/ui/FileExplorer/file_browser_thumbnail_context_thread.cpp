...
void FileBrowserThumbnailThread::start(DoneFn on_done) {
	...
	for (int i = 0; i < k_worker_count; ++i) {
		...
		m_workers.push_back(std::make_unique<ManagedThread>(cfg, [this](std::stop_token const &st, ManagedThread &self) {
			worker_iteration(st, self);
		}));
	}
}
...
bool FileBrowserThumbnailThread::render_video(std::filesystem::path const &file, std::filesystem::path const &out_png,
	std::vector<std::uint8_t> &out_rgba, std::stop_token const &st, ManagedThread &self) {
	...
	while (!st.stop_requested() && std::chrono::steady_clock::now() < deadline) {
		...
		if (frame_ready.exchange(false)) {
			...
			mpv_render_param rp[]    = {{MPV_RENDER_PARAM_SW_SIZE, size},
				   ...
				   {MPV_RENDER_PARAM_SW_STRIDE, &stride}, {MPV_RENDER_PARAM_SW_POINTER, out_rgba.data()},
				   {MPV_RENDER_PARAM_INVALID, nullptr}};
	...
	if (good && !st.stop_requested()) {
		...
		stbi_write_png(out_png.string().c_str(), k_thumb_w, k_thumb_h, 4, out_rgba.data(), k_thumb_w * 4);
