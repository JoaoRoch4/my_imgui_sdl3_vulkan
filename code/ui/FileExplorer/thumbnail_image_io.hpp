...
decode_stb(std::filesystem::path const &file, int desired_channels = 4) {
	...
	int width    = 0;
	int height   = 0;
	int channels = 0; // channels actually present in the file (ignored; we force desired)
	...
	unsigned char *pixels = stbi_load(file.string().c_str(), &width, &height, &channels, desired_channels);
