...
void ImGui::FileBrowser::Display() {
    ...
    const bool doClose = Button("cancel") || shouldClose_
...
const std::filesystem::path& ImGui::FileBrowser::GetDirectory() const noexcept { return currentDirectory_; }
...
void ImGui::FileBrowser::SetContextMenuCallback(std::function<void(const std::filesystem::path&)> cb) {
    contextMenuCallback_ = std::move(cb);
}

void ImGui::FileBrowser::SetRebuildThumbnailCallback(std::function<void(const std::filesystem::path&)> cb) {
...
void ImGui::FileBrowser::RebuildThumbnail(const std::filesystem::path &path) {
...
void ImGui::FileBrowser::PollScan() {
	...
	for (const auto& name : selectedFilenames_)
	    if (std::any_of(fileRecords_.begin(), fileRecords_.end(), [&](const FileRecord& rec) {
...
void ImGui::FileBrowser::RebuildView() {
	...
	std::sort(fileRecords_.begin() + 1, fileRecords_.end(), [this](const FileRecord& a, const FileRecord& b) -> bool {
...
int ImGui::FileBrowser::CompareByField(const FileRecord& a, const FileRecord& b) const {
...
void ImGui::FileBrowser::SetCurrentDirectoryUncatched(const std::filesystem::path& pwd) {
    const std::filesystem::path target = absolute(pwd);
...
bool ImGui::FileBrowser::SetCurrentDirectoryInternal(const std::filesystem::path& dir,
