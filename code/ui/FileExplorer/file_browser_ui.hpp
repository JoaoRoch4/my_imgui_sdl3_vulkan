...
enum ImGuiFileBrowserFlags_ : std::uint32_t {
	...
	ImGuiFileBrowserFlags_EnterNewFilename = 1 << 1, // allow user to enter new filename when
													 // selecting regular file
	ImGuiFileBrowserFlags_NoModal     = 1 << 2, // file browsing window is modal by default. specify
											// this to use a popup window
	ImGuiFileBrowserFlags_NoTitleBar  = 1 << 3, // hide window title bar
	ImGuiFileBrowserFlags_NoStatusBar = 1 << 4, // hide status bar at the bottom of browsing window
	ImGuiFileBrowserFlags_CloseOnEsc  = 1 << 5, // close file browser when pressing 'ESC'
	ImGuiFileBrowserFlags_CreateNewDir      = 1 << 6, // allow user to create new directory
	ImGuiFileBrowserFlags_MultipleSelection = 1 << 7, // allow user to select multiple files. this
													  // will hide
													  // ImGuiFileBrowserFlags_EnterNewFilename
	ImGuiFileBrowserFlags_HideRegularFiles  = 1 << 8, // hide regular files when
													 // ImGuiFileBrowserFlags_SelectDirectory is
													 // enabled
	ImGuiFileBrowserFlags_ConfirmOnEnter    = 1 << 9, // confirm selection when pressing 'ENTER'
	ImGuiFileBrowserFlags_SkipItemsCausingError
	= 1 << 10, // when entering a new directory, any error will interrupt
	...
	ImGuiFileBrowserFlags_EditPathString = 1 << 11, // allow user to directly edit the whole path
													// string
	ImGuiFileBrowserFlags_Window         = 1 << 12, // render as a plain ImGui::Begin window (not a
											// popup/modal). the window never steals focus and stays
											// open until explicitly closed via Close() or the
											// title-bar X button.
};
...
class FileBrowser {
	public:
		...
		bool SetPwd(std::filesystem::path const& dir = std::filesystem::current_path()) {
			return SetDirectory(dir);
		}

		// get current browsing directory
		[[nodiscard]] std::filesystem::path const& GetDirectory() const noexcept;

		// legacy interface. use GetDirectory instead.
		[[nodiscard]] std::filesystem::path const& GetPwd() const noexcept {
			return GetDirectory();
		}
		...
		void SetRecentDirectories(std::vector<std::filesystem::path> const& directories);
		...
		void SetHoverFileCallback(std::function<void(std::filesystem::path const&)> cb);
		...
		void SetContextMenuCallback(std::function<void(std::filesystem::path const&)> cb);
		...
		void SetRebuildThumbnailCallback(std::function<void(std::filesystem::path const&)> cb);
		...
		void SetOpenFileCallback(std::function<void(std::filesystem::path const&)> cb);
		...
		void RebuildThumbnail(std::filesystem::path const& path);
		...
		// Inline animated-GIF playback mode (a runtime "run config"; combo in the toolbar).
		// Must mirror FileBrowserThumbnailContext::GifPlayback (same order) — it forwards there.
		enum class GifPlayback { HoverOnly, AllVisible, Hybrid };
		void                      SetGifPlayback(GifPlayback mode) noexcept;
		[[nodiscard]] GifPlayback GetGifPlayback() const noexcept;

		/**
		 * @brief Retrieves the real aspect ratio of the currently selected file's thumbnail.
		 * @return The calculated float aspect ratio (width / height). Returns 16:9 (1.7778f) as a
		 * safe fallback.
		 */
	...
	private:
		...
		void TouchRecentDirectory(std::filesystem::path const& dir);

		void ToolTip(std::string_view const& s);
		...
		[[nodiscard]] int CompareByField(FileRecord const& a, FileRecord const& b) const;

		void SetCurrentDirectoryUncatched(std::filesystem::path const& pwd);

		bool SetCurrentDirectoryInternal(std::filesystem::path const& dir,
			std::filesystem::path const&                              preferredFallback);

		[[nodiscard]] bool IsExtensionMatched(std::filesystem::path const& extension) const;
		...
		// Display() is split into focused steps that each run once per frame, in
		// order. Per-frame scratch shared between them lives in the members marked
		// "Per-frame Display() scratch" below (searchLower_, pendingDir_, …).
		[[nodiscard]] bool IsWindowMode() const noexcept;
		[[nodiscard]] bool BeginContainer(); // open popup/modal/window; false => skip body
		void               EndContainer(); // End()/EndPopup() for the active container
		void               CloseContainer(); // ask the active container to close

		void DrawDirectoryRow(); // breadcrumb buttons OR the path edit field
		void DrawBreadcrumbBar();
		void DrawPathEditField();
		void DrawTopButtons(); // refresh + create-new-directory
		void DrawToolbar(); // sort / media / tag / checkboxes / view-mode
		void DrawSearchBar();
		void DrawFileListChild(); // the scrolling "ch" child: keyboard + view dispatch
		void DrawPermDeleteModal();
		void DrawFilenameInputRow();
		void HandleSelectAllShortcut();
		void DrawActionsRow(); // ok / cancel / status / type-filter combo
		void DrawTypeFilterCombo();

		// File-list child internals.
		void HandleThumbnailWheelZoom();
		void HandleListKeyboard(); // focus-gated dispatch for the three handlers below
		void HandleSmoothScroll(ImGuiIO const& io, bool shift);
		void HandleImageNavKeys();
		void HandleDeleteKeys(bool shift);
		void RenderListView();
		void RenderGridView();
		void RenderMasonryView();

		// Per-row helpers shared by all three views (replace the old copy-paste).
		[[nodiscard]] bool PassesDisplayFilter(FileRecord const& rsc) const;
		[[nodiscard]] bool IsImageNavCandidate(FileRecord const& r) const;
		[[nodiscard]] bool MatchesMediaFilter(std::filesystem::path const& extension) const;
		void               HandleSelectionClick(FileRecord const& rsc, unsigned int idx);
		void               HandleActivate(FileRecord const& rsc, bool focusPrevAfterDirNav = false);
		void               FireRowCallbacks(FileRecord const& rsc, bool hovered);
		void FillRangeSelection(unsigned int a, unsigned int b, bool wantDir, bool requireExtMatch);
		[[nodiscard]] ImTextureID ThumbnailFor(FileRecord const& rsc);
		[[nodiscard]] float       ThumbnailAspectRatio(FileRecord const& rsc) const noexcept;
		void DrawThumbnailFitted(ImTextureID tex, FileRecord const& rsc, ImVec2 cell) const;
		...
		std::function<void(std::filesystem::path const&)> hoverFileCallback_;
		std::function<void(std::filesystem::path const&)> contextMenuCallback_;
		std::function<void(std::filesystem::path const&)> rebuildThumbnailCallback_;
		std::function<void(std::filesystem::path const&)> openFileCallback_; // A/D image-open
		// W/S keyboard scroll step (px); Shift+W/S page-scrolls. Editable in Runtime Config.
		// Default 40 matches the config fallback; was previously left uninitialized (read
		// garbage on any frame that ran before external window-state load called SetScrollStep).
		...
		// ── Per-frame Display() scratch (reset at the top of every Display()) ──────
		// Promoted from Display()'s locals so the decomposed Draw*/Handle*/Render*
		// helpers can share them without threading parameters through every call.
		std::filesystem::path pendingDir_; ///< deferred SetDirectory() target
		bool                  pendingDirSet_    = false;
		bool                  inputTextFocused_ = false; ///< a text field grabbed focus this frame
		std::string           searchLower_; ///< cached ToLower(searchStr_) for this frame
		bool                  hideRegularFiles_ = false; ///< SelectDirectory + HideRegularFiles

		std::vector<std::string> availableTags_; ///< union of user.xdg.tags across all files in
												 ///< currentDirectory_
