...
bool MainMenuBar::Draw(MainMenuBar::MenuContext const &mc) {
	...
	bool const   m_use_video_player_placebo = mc.use_video_player_placebo;
	...
	if (ImGui::BeginMenu("File")) {
		...
		if (!history.empty() && ImGui::BeginMenu("Recent")) {
			auto const result = RecentHistoryMenu::draw_entries(history,
				{
					.on_open =
					...
					.on_hover
					= [&](WindowStateToml::ImageHistoryEntry &entry) { m_history_preview->draw_for_hover(entry); },
					.on_after_item =
						[&](WindowStateToml::ImageHistoryEntry &entry) {
							bool const is_video
							...
							if (is_video) {
								if (auto const r = m_video_context_menu->draw_for_item(entry);
							...
							if (auto const erase = HistoryContextMenu::draw_for_item(entry.source)) {
			...
			if (result.shown < static_cast<int>(history.size())) {
				ImGui::Separator();
				ImGui::TextDisabled("(%zu more not shown)", history.size() - static_cast<size_t>(result.shown));
			}
