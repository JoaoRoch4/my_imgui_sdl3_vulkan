...
void AppCoordinator::Setup(StyleEditor *style_editor, SDL_Window *window,
  ...
  m_app_state->setup(m_history_mgr, m_config_runtime, m_history_preview,
                     m_opened_files_window, m_video_downloader);
  ...
  m_load_handler->setup(m_viewer, m_video_player, m_video_player_placebo,
                        m_bulk_image_open, m_history_mgr, m_video_downloader,
                        m_opened_files_window, m_vk,
                        m_use_video_player_placebo);
...
void AppCoordinator::ApplyHistory(const WindowStateToml &state) {
  m_app_state->apply_history(state);
}

void AppCoordinator::ApplyRuntimeConfig(const WindowStateToml &state) {
  ...
  if (state.file_explorer_thumb_size)
    browser.SetThumbnailSize(
        ImVec2{state.file_explorer_thumb_size->x,
               state.file_explorer_thumb_size->y});
  if (state.file_explorer_grid_thumb_size)
    browser.SetGridThumbnailSize(
        ImVec2{state.file_explorer_grid_thumb_size->x,
               state.file_explorer_grid_thumb_size->y});
...
bool AppCoordinator::LoadOpenedFilesHistoryFromToml(
    const std::filesystem::path &file_path) {
  return m_app_state->load_opened_files_history_from_toml(file_path);
}

void AppCoordinator::SetStatePath(const std::filesystem::path &file_path) {
...
void AppCoordinator::ExportRuntimeConfig(WindowStateToml *state) const {
  ...
  {
    const ImVec2 ts = browser.GetThumbnailSize();
    state->file_explorer_thumb_size = WindowStateToml::Vec2Toml{ts.x, ts.y};
  }
  {
    const ImVec2 gs = browser.GetGridThumbnailSize();
...
void AppCoordinator::SetDownloadCacheDir(const std::filesystem::path &dir) {
  m_app_state->set_download_cache_dir(dir);
}

void AppCoordinator::HandleSdlEvent(const SDL_Event &event) {
