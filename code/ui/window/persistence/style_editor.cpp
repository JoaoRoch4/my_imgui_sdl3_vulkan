...
WindowStateToml::Vec4Toml ToToml(const ImVec4& value) // NOLINT(readability-convert-member-functions-to-static)
{
...
{
    auto clamp = [](float v) { return static_cast<int>(v * 255.0f + 0.5f < 0 ? 0 : v * 255.0f + 0.5f > 255 ? 255 : v * 255.0f + 0.5f); };
...
ImVec4 FromToml(const WindowStateToml::Vec4Toml& value) //NOLINT(readability-convert-member-functions-to-static)
{
...
{
    return { value.r / 255.0f, value.g / 255.0f, value.b / 255.0f, value.a / 255.0f }; // NOLINT
}

bool HasPersistedStyle(const WindowStateToml::StyleToml& style)
{
    return style.font_size_base > 0.0f
        && style.curve_tessellation_tol > 0.0f
        && style.circle_tessellation_max_error > 0.0f
        && style.alpha > 0.0f;
}
...
void ApplyStyleToml(ImGuiStyle* style, const WindowStateToml::StyleToml& s)
{
    ...
    style->WindowBorderHoverPadding          = std::max(s.window_border_hover_padding, 0.001f);
    ...
    { const ImGuiDir d = static_cast<ImGuiDir>(s.window_menu_button_position);
      style->WindowMenuButtonPosition = (d == ImGuiDir_None || d == ImGuiDir_Left || d == ImGuiDir_Right) ? d : ImGuiDir_Left; }
    ...
    style->TabCloseButtonMinWidthUnselected  = s.tab_close_button_min_width_unselected;
    ...
    style->TableAngledHeadersTextAlign       = FromToml(s.table_angled_headers_text_align);
    { const ImGuiTreeNodeFlags tf = static_cast<ImGuiTreeNodeFlags>(s.tree_lines_flags);
      style->TreeLinesFlags = (tf == ImGuiTreeNodeFlags_DrawLinesNone || tf == ImGuiTreeNodeFlags_DrawLinesFull || tf == ImGuiTreeNodeFlags_DrawLinesToNodes) ? tf : ImGuiTreeNodeFlags_DrawLinesNone; }
    ...
    { const ImGuiDir d = static_cast<ImGuiDir>(s.color_button_position);
      style->ColorButtonPosition = (d == ImGuiDir_Left || d == ImGuiDir_Right) ? d : ImGuiDir_Right; }
    ...
    style->CircleTessellationMaxError        = std::max(s.circle_tessellation_max_error, 0.001f);
    for (int i = 0; i < ImGuiCol_COUNT; ++i)
    ...
    style->HoverFlagsForTooltipMouse         = static_cast<ImGuiHoveredFlags>(s.hover_flags_for_tooltip_mouse);
    style->HoverFlagsForTooltipNav           = static_cast<ImGuiHoveredFlags>(s.hover_flags_for_tooltip_nav);
}
...
void StyleEditor::SetFonts(std::vector<std::pair<std::string, ImFont*>> fonts)
{
...
void StyleEditor::Draw()
{
    ...
    if (!m_fonts.empty())
    {
        ...
        for (const auto& entry : m_fonts)
            { font_items += entry.first; font_items += '\0'; }
        if (ImGui::Combo("Font", &font_index, font_items.c_str()))
        {
            m_current_font_name        = m_fonts[static_cast<size_t>(font_index)].first;
            ImGui::GetIO().FontDefault = m_fonts[static_cast<size_t>(font_index)].second;
        }
    }

    int preset_index = 0;
    if (m_current_preset_name == kMicaPresetName)         preset_index = 1;
    else if (m_current_preset_name == kMicaColorfulPresetName) preset_index = 2;
    if (ImGui::Combo("Preset", &preset_index, "ImGui Default\0Mica\0Mica Colorful\0"))
    {
        if (preset_index == 1)      ApplyPresetByName(kMicaPresetName);
        else if (preset_index == 2) ApplyPresetByName(kMicaColorfulPresetName);
        else                        ApplyPresetByName(kImGuiDefaultPresetName);
    }
...
void StyleEditor::ApplyPresetByName(const std::string& preset_name)
{
    ...
    if (preset_name == kMicaColorfulPresetName)
    {
        ...
        style.Colors[ImGuiCol_TitleBgCollapsed]         = ImVec4(0.49f, 0.46f, 0.46f, 0.20f);
        ...
        style.Colors[ImGuiCol_SliderGrabActive]         = ImVec4(0.00f, 0.35f, 1.00f, 1.00f);
        ...
        style.Colors[ImGuiCol_ResizeGripActive]         = ImVec4(0.12f, 0.00f, 0.99f, 0.97f);
        ...
        style.Colors[ImGuiCol_TabDimmedSelected]        = ImVec4(0.39f, 0.38f, 0.38f, 0.91f);
