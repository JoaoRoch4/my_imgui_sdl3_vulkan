...
std::string FieldCommentFor(const std::string &section_name,
  ...
  if (key == "file_explorer_show_thumbnails")
    return "# Show file thumbnails in File Explorer (inline list and grid).";
  if (key == "file_explorer_keep_open")
    return "# Keep File Explorer open after opening a file.";
  if (key == "file_explorer_thumb_size")
