...
struct WindowStateToml {
  ...
  struct ImageHistoryEntry {
    ...
    std::string thumbnail_path; // path to cached thumbnail PNG (empty = not yet
                                // generated)
    std::string cached_path;    // local downloaded copy of a URL video (empty =
                                // not cached)
    ...
    bool startup_restore =
        false; // reopen this entry automatically on next startup
  };
  ...
  StyleToml style;
  std::vector<ImageHistoryEntry>
      image_history; // newest first, persisted across runs
