...
void VideoSeekPreview::setup(vulkan_context *vk, VulkanUploadContext *uploader, std::string const &source) {
	prepare(vk, uploader, source);
	ensure_active();
}

void VideoSeekPreview::prepare(vulkan_context *vk, VulkanUploadContext *uploader, std::string const &source) {
...
void VideoSeekPreview::shutdown() {
	bool const is_inactive = !m_thread && m_mpv == nullptr && m_render_ctx == nullptr && m_image == VK_NULL_HANDLE
		&& m_image_memory == VK_NULL_HANDLE && m_image_view == VK_NULL_HANDLE && m_sampler == VK_NULL_HANDLE
		&& m_descriptor_set == VK_NULL_HANDLE && m_vk == nullptr && m_uploader == nullptr;
	if (is_inactive)
		return;
...
void VideoSeekPreview::init_mpv(std::string const &source) {
	...
	auto    *api_type_payload = static_cast<void *>(std::any_cast<char *>(api_type_value));
	...
	char const *cmd[] = {"loadfile", source.c_str(), nullptr};
...
bool VideoSeekPreview::create_gpu_resources(vulkan_context *vk) {
	...
	alloc.memoryTypeIndex
		= find_memory_type(vk->physical_device, req.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
...
void VideoSeekPreview::start_thread() {
	...
	m_thread = std::make_unique<ManagedThread>(cfg, [this](std::stop_token const & /*stoken*/, ManagedThread & /*self*/) {
		...
		if (req >= 0.0 && m_mpv && m_render_ctx) {
			...
			char const *cmd[] = {"seek", t, "absolute+exact", nullptr};
			mpv_command_async(m_mpv, 0, cmd);
			...
			if (m_frame_dirty) {
				...
				void    *sw_format_payload = static_cast<void *>(std::any_cast<char *>(sw_format_value));
...
void VideoSeekPreview::update() {
	...
	std::lock_guard lock(m_buf_mutex);
	m_uploader->upload_to_image(m_buf.data(), m_image, (uint32_t)m_w, (uint32_t)m_h);
}
