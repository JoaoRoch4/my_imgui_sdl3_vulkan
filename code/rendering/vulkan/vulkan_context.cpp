...
void vulkan_context::setup(std::vector<char const *> instance_extensions) {
	VkResult err;
	...
	for (auto const &get_device : devices) {
		VkPhysicalDeviceProperties deviceProperties;
		vkGetPhysicalDeviceProperties(get_device, &deviceProperties);
		...
		for (char const *ext : requested_device_extensions) {
			if (!ext)
				continue;
...
VkResult vulkan_context::queue_present(VkPresentInfoKHR const *present_info) {
	std::scoped_lock lock(queue_mutex);
